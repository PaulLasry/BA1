...
\end{example}
\begin{remark}
    Si $\int_{-\infty}^{+\infty}f(x)dx$ converge, i.e. si les deux limites existent, alors cette intégrale vaut : \[\underbrace{\lim_{u\to + \infty}\int_{-u}^{u} f(x)dx}_{\textstyle\text{valeur de cauchy}}\]
\end{remark}
\begin{example}[5]
    \begin{align*}
...
\end{example}
\begin{property}[Comparaison intégrales / séries]\label{prop:comp_integrale_series}
...
\newpage
\begin{example}
    \[\sum_{n =2}^{\infty} \frac{1}{n(\underbrace{\log(n)}_{u})^p} \; \text{ converge } \iff I=\int_{2}^{+\infty}\frac{1}{x\left(\log(x)\right)^p}dx \; \text{ converge}\]
    Voyons la valeur de l'intégrale indéfinie :
    \begin{align*}
    ...
    \end{align*}
    $I$ converge si et seulement si $p>1$, donc par la propriété \ref{prop:comp_integrale_series}
