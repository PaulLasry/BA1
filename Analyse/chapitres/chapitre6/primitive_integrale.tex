\begin{definition}[Primitive]
    Soit $f:I\to\R$ une fonction continue où $I$ est intervalle. Une \textbf{primitive} de $f$ est une fonction $F:I\to\R$ telle que \[F'(x)=f(x), \quad \forall x\in I.\]
\end{definition}
\begin{remark}
...
\end{remark}
\textbf{Notation :} $\displaystyle\int f(x) dx$ est l'ensemble de \underline{toutes} les primitives de $f$. Donc :
\[\int f(x)dx = \{F(x)+c \mid c\in\R\}\] où $F$ est une primitive de $f$. Abus de notation : \[\int f(x) dx = F(x)+c\]
\begin{figure}
    \[
    \begin{array}{|c|c||c|c|}
        \hline
        ...
        \hline\hline
        x & \frac{1}{2}x^2 + C & 1 + \tan^2(x) = \frac{1}{\cos^2(x)} & \tan(x) + C \\
        \hline
        x^r \ (r \neq -1) & \frac{1}{r+1} x^{r+1} + C & \frac{1}{1 + x^2} & \arctan(x) + C \\
        \hline
        \frac{1}{x} & \log |x| + C & \frac{1}{\sqrt{1 - x^2}} & \arcsin(x) + C \\
        \hline
        e^x & e^x + C & \frac{-1}{\sqrt{1 - x^2}} & \arccos(x) + C \\
        \hline
        \sin(x) & -\cos(x) + C & & \\
        \hline
        \cos(x) & \sin(x) + C & & \\
    ...
    \end{array}
    \]
    \caption{Primitives de fonctions de base}
\end{figure}

\begin{remark}
    L'intégrale $\int f(x)dx$ s'appelle \textbf{intégrale indéfinie} de $f$.
\end{remark}
...
\end{center}
Approx 1 (inférieure):
\[\text{Aire} \approx    \sum_{i = 1}^{n} (x_i - x_{i-1}) \cdot (\inf_{x\in[x_{i-1}, x_i]} f(x))\]
Approx 2 (supérieure):
\[\text{Aire} \approx  \sum_{i = 1}^{n} (x_i - x_{i-1}) \cdot (\sup_{x\in[x_{i-1}, x_i]} f(x))\]
\begin{definition}[Intégrale de Riemann]
    Soit $f:[a,b]\to\R$ est \textbf{intégrable} (au sens de Riemann) si \[\sup\{\text{Approx 1}\} = \inf\{\text{Approx 2}\} = A \in\R\]
...
\begin{property}[Intégrale de Riemann]
    Soient $f, g : [a,b]\to\R$ intégrables au sens de Riemann et $\alpha, \beta \in\R$. Alors :
    \begin{enumerate}
        \item $\displaystyle\int_{a}^{b}(\alpha f(x) + \beta g(x)) dx = \alpha \int_a^b f(x) dx + \beta \int_a^b g(x) dx$ \itemtitle{Linéarité}
        \item Si $a <u <b$, alors : $\displaystyle\int_a^b f(x) dx = \int_a^u f(x) dx + \int_u^b f(x) dx$ \itemtitle{Additivité}
        \item Si $f(x)\leq g(x), \forall x\in[a,b]$, alors $\displaystyle\int_a^b f(x) dx \leq \int_a^b g(x) dx$ \itemtitle{Monotonie}
    \end{enumerate}
\end{property}
\begin{proof}[Preuve]
    Technique, mais :
...
\begin{theorem}[Théorème de la moyenne]
    Soit $f:[a, b]\to\R$ continue. Alors il existe $u\in\left]a, b\right
    [$ tel que : \[\int_{a}^{b}f(x)dx = f(u)\cdot(b-a)\]
\end{theorem}
\begin{proof}
    On prend $m = \min_{x\in[a, b]} f(x)\leq f(x) \leq M = \max_{x\in[a, b]} f(x)\quad\forall x\in\left[a, b\right]$. Donc :
    \[m(b-a) \leq \int_{a}^{b} f(x) dx \leq M(b-a)\]
    Ainsi : \[m \leq \frac{1}{b-a} \int_{a}^{b}f(x)dx\leq M\]
    Par le théorème des valeurs intermédiaires, il existe $u\in\left]a, b\right[$ tel que $f(u) = \frac{1}{b-a} \int_{a}^{b}f(x)dx$.
\end{proof}
\begin{remark}
...
\begin{proof}
    ...
    \textbf{Parie 1 :} On dérive $G$ : \begin{align*}
        ...
        &= \lim_{h\to 0}\frac{1}{h} \int_{x}^{x+h}f(t)dt = \lim_{h\to0}f(u)\left((x+h)-x\right) = f(x)h\\
        &= \lim_{h\to 0} \frac{1}{h}\underbrace{f(u)h}_{\in\left]x, x+h\right[} = \lim_{u\to x}f(x)
    \end{align*}
