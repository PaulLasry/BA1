...
\end{proof}
\begin{remark}
    $f$ continues $\nRightarrow$ $f$ dérivable.
\end{remark}
\begin{example}
    $f(x) = \left|x\right|$, $x_0=0$. Alors $f$ est continue en $0$ mais on a \[\lim_{h\downarrow0}\frac{f(0+h)-f(0)}{h} = \lim_{h\downarrow0}\frac{|h|}{h} = 1\neq -1 = \lim_{h\uparrow0}\frac{|h|}{h} = \lim_{h\uparrow0}\frac{f(0+h)-f(0)}{h}\]
    Ainsi, la limite n'existe pas, donc $f$ n'est pas dérivable en $0$.
\end{example}
\begin{property}[Opérations algébriques]
    ...
    \begin{enumerate}
        ...
        \item $\displaystyle(f\cdot g)'(x_0) = f'(x_0)\cdot g(x_0) + f(x_0)\cdot g'(x_0)$\label{prop:derive_op_algebrique_2}
        \item $\displaystyle\left(\frac{f}{g}\right)'(x_0) = \frac{f'(x_0)\cdot g(x_0) - f(x_0)\cdot g'(x_0)}{(g(x_0))^2}$, si $g(x_0)\neq 0$\label{prop:derive_op_algebrique_3}
    \end{enumerate}
\end{property}
\begin{property}[Dérivées de fonction usuelles]
    ...
    \begin{enumerate}[start=0]
            ...
            \begin{itemize}
                ...
                \item Supposons vrai pour $n$, montrons pour $n+1$ :
                    \begin{align*}
                        ...
                        f'(x) &\overset{\text{Prop \ref{prop:derive_op_algebrique_2}}}{=} (x^n)' \cdot x + x^n \cdot (x)'\\
        ...
        \item $f(x)=x^{-n}$, $n\in \N^*, x\neq0$\[\implies f(x)=\frac{1}{x^n} \overset{\text{Prop \ref{prop:derive_op_algebrique_3}}}{\implies} f'(x) = -n\cdot x^{-n-1}\]
    \end{enumerate}
\end{property}
\begin{property}[Dériviées de composées]\label{prop:derivee_composee}
    ...
    \end{align*}
\end{property}
\begin{proof}[Preuve de la propriété \ref{prop:derivee_composee}]
...
\end{proof}
\begin{property}[Dériviées des réciproques]\label{prop:derivee_reciproques}
...
\end{property} 
\begin{proof}[Preuve de la propriété \ref{prop:derivee_reciproques}]
    On admet que $f^{-1}$ est dérivable sur tout $B$. On dérive l'equation $f(f^{-1}(x)) = x$ des deux cotés :
    \begin{align*} 
        x = \left(f\circ f^{-1}\right)(x)\iff 1 =& f(f^{-1}(x))' \overset{\text{Prop \ref{prop:derivee_composee}}}{=} f'\left(f^{-1}(x)\right)\cdot \left(f^{-1}\right)'(x)\\ 
        \iff& \frac{1}{f'\left(f^{-1}(x)\right)} = (f^{-1})'(x) 
    \end{align*} 
\end{proof} 
\begin{example}[1] $f(x) = \sqrt[n]{x} = f^{-1}(x)$ où $f(x)=x^n$, où $x>0$ \begin{align*} x &= f(g(x)) = \left(\sqrt[n]{x}\right)^n\\
        &\implies 1 = n(\sqrt[n]{x})^{n-1}\cdot (\sqrt[n]{x})'\\
        &\implies (\sqrt[n]{x})' = \frac{1}{n(\sqrt[n]{x})^{n-1}} = \frac{1}{n}x^{\frac1n -1}
    \end{align*} 
    On montre de manière analogue que $\displaystyle\left(x^{\frac{p}{q}}\right) = \frac{p}{q}x^{\frac{p}{q}-1}$ pour tout $\frac{p}{q} \in \Q$ et on verra que c'est aussi vrai pour tout réel.
\end{example}
\begin{example}[2]
    $\displaystyle\arcsin'(x) = \frac{1}{\cos(\arcsin(x))}$, $x\in \left]-1,1\right[$, car $f = \sin(x)$, donc $f'(x) = \cos(x)$
        \begin{center}
...
\end{example}
\begin{definition}
    $\displaystyle\lim_{h \downarrow 0}\frac{f(x+h)-f(h)}{h}$ est la dérivée à droite, si $h\uparrow 0$, c'est la dérivée à gauche.
\end{definition}
\begin{property}
    $f$ est dérivable en $x_0$ si et seulement si les deux dérivées latérales existent ($\in\R$) et sont égales.
\end{property}
\begin{example}[1]
...
\begin{example}[2]
    ...
    $f'(0)$ n'existe pas car $\notin\R$, mais les deux dérivées latérales sont égales à $+\infty$.
\end{example}
...
\begin{definition}[Clsses de fonctions dérivables]
    ...
    \begin{align*}
        &D^n(I) = \left\{f : I\to \R \mid f \text{ est $n$ fois dérivable sur } I\right\}\\
        &C^n(I) = \left\{f : I\to \R \mid f \text{ est $n$ fois dérivable sur et $f^{(n)}$ est continue sur } I\right\}
    \end{align*}
    On pose $C^{\infty}(I) = \left\{f : I\to\R\mid f^{(n)}\text{ existe pour tour } n\in \N\right\}$.
\end{definition}
\begin{remark}
    \noindent
    \begin{itemize}
...
\begin{remark}
    \noindent
    \begin{itemize}
        \item $x^n\left|x\right|$ est dans $C^{n}(\R)\setminus D^{n+1}(\R)$
        \item $x\left|x\right| \begin{aligned}[t]
            &\in C^{\infty}\Big(\left]0, +\infty\right[\Big)\\
            &\in C^{\infty}\Big(\left]-\infty, 0\right[\Big)
        \end{aligned}$
    \end{itemize}
\end{remark}
...
\begin{theorem}[Théorhème de Rolle]
    Soit $f:[a,b]\to\R$ continue sur $[a,b]$ et dérivable sur $]a,b[$. On suppose que $f(a)=0=f(b)$. Alors il existe $u\in \left]a, b\right[$ tel que $f'(u)=0$.
        \begin{center}
            \includegraphics[width=0.5\textwidth]{rolle.png}           
        \end{center}
\end{theorem}
\begin{proof}[Preuve par le TVI]
    Par le TVI (\ref{thm:TVI}), $f$ atteint son max $M=\max_{x\in[a,b]} f(x)$, on le suppose supérieur à $0$. Alors il existe $u\in \left]a, b\right[$ tel que :
    \begin{align*}
    ...
    Donc $f'(u)=0$.
\end{proof}
\begin{theorem}[Théorhème des acroissements finis -- TAF]\label{thm:TAF}
    Soit $f:[a, b]\to \R$ continue, dérivable sur $]a, b[$. Alors il existe $u\in ]a, b[$ tel que :
    \begin{align*}
...
\begin{proof}[Preuve]
    En exercice.
\end{proof}
\begin{property}[Application du TAF (\ref{thm:TAF})]
    ...
    \begin{enumerate}
        ...
        \begin{proof}[Preuve]
            ...
            Par le TAF, $\exists u\in \left]c,d\right[$ tel que \[f'(u) = \frac{f(d)-f(c)}{d-c} \neq 0\] ce qui est une contradiction.
        \end{proof}
        \item Si $g:[a, b]\overset{\text{continue}}{\longrightarrow }\R$ dérivable sur $]a,b[$ et $f'(x)=g'(x)$ pour tout $x\in \left]a,b\right[$, alors $f(x) = g(x) + c$ pour un certain $c\in\R$ et tout $x\in [a,b]$.
        \item Si $f'(x)\geq 0$ pour tout $x\in \left]a,b\right[$, alors $f$ est croissante sur $[a,b]$.
        \item Si $f'(x)\leq 0$ pour tout $x\in \left]a,b\right[$, alors $f$ est décroissante sur $[a,b]$.
        \item Si $f'(x)>0$ pour tout $x\in \left]a,b\right[$, alors $f$ est strictement croissante sur $[a,b]$.
        \item Si $f'(x)<0$ pour tout $x\in \left]a,b\right[$, alors $f$ est strictement décroissante sur $[a,b]$.
    \end{enumerate}
\end{property}
\begin{remark}
...
\begin{theorem}
    Il existe une \uline{unique} fonction $f:\R\to\R$ telle que :
    \begin{align*}
...
\begin{proof}[Preuve]
    ...
    \begin{enumerate}
        \item $f(x)\neq0 \quad\forall x\in \R$. On pose $h(x)=f(x)\cdot f(-x)$. On calcule la dérivée de $h$:
            \begin{align*}
...
\begin{definition}[Fonction exponentielle]
    Cette fonction s'appelle la fonction \uline{ex\-po\-nen\-tielle}, notée $\exp(x)$ (et plus tard $e^x$).
\end{definition}
\begin{property}[$\exp(x)$]
    \noindent
    \begin{enumerate}
        \item $\exp'(x) = \exp(x)$ et $\exp(0)=1$ nous donne \[\exp \in C^\infty(\R)\]
        \item $\exp(x)\neq 0 \quad \forall x\in \R$ et $\exp(-x) = \displaystyle\frac{1}{\exp(x)}$
        \item la fonction exponentielle est strictement croissante
        \item $\exp(x+y) = \exp(x)\cdot\exp(y) \quad \forall x\in\R$
...
\begin{remark}
    ...
    \begin{itemize}
        ...
        \item surjective (coréstraintes à $\left]0, +\infty\right[$)
    \end{itemize}
...
\begin{definition}[Logarithme]
    ...
    \begin{align*}
        \log : \left]0, + \infty\right[ &\longrightarrow \R\\
...
\begin{property}[Logarithme]
    \noindent
    \begin{enumerate}
        \item $\mathcal{D}(\log) = \left]0, + \infty\right[$ et $\Im(\log) = \R$. 
        \item $\log(1) = 0$. On a :
            \begin{align*}
                &x = \exp(\ln(x)) \\
                \iff& 1 = \exp'(\ln(x))\cdot\ln'(x) = x\ln'(x)\\
                \implies& \ln'(x) = \frac{1}{x}\\
                \implies& \ln \in C^\infty\left(\left]0, +\infty\right[\right)
            \end{align*} 
...
\begin{theorem}[Règle de Bernouilli-l'Hospital]
    Soit $x_0$ et $f, g: A\to\R$, où $A=\left]x_0-d, x_0+d\right[\setminus{\{x_0\}}$ est un voisinage de $x_0$. Si :
    \begin{enumerate}
...
\begin{example}[2]
    \begin{align*}
        ...
        \end{cases}
    \end{align*}
    Cela montre que $\log(x)$ croît moins vite que tout polynôme (de degré supérieur à $0$).
\end{example}
\begin{example}[3]
    \begin{align*}
        \lim_{x\to 0}\cos(2x)^{\frac{3}{x^2}} &= \lim_{x\to 0}\exp\left(\frac{\log(\cos(2x)\cdot3)}{x^2}\right)\\ 
...
\end{remark}
\begin{property}[Fonctions $\in C^1$ par morceaux]
...
Une suite définie par récurrence est une suite $(a_n)_{n\in\N}$ définie par $a_0 = \text{valeur fixée}$ et $a_{n+1} = g(a_n)$, pour une fonction $g:D\to\R$.
\begin{remark}[Important]
    ...
    \end{align*}
    Donc $l$ est forcément une sol de l'équation $\underline{x = g(x)}$. On supposera en général que $g(x)$ est continue et même $C^1$ sur un intervalle. 
\end{remark}
\begin{example}
...
\begin{definition}
    Soit $f\in C^n(I)$ où $I$ est un intervalle ouvert et $x_0\in I$. Le polynôme de Taylor de $f$ d'ordre $n$ en $x_0$ est l'unique polynôme $p(x)$ de degré inférieur ou égal à $n$ dont les dérivées d'ordre $x_0$ sont les mêmes que celles de $f$ jusqu'à l'ordre $n$.
\end{definition}
\begin{example}
...
\begin{definition}[Développement limité]
    ...
    pour tout $x\in I$ où $r_n(x)$ vérifie
     \begin{itemize}
        \item Pour tout $c > 0$, $|r_n(x)| \leq c\cdot|x-x_0|^n$ dans un voisinage de $x_0$.
...
\begin{remark}[Estimation du reste]
    On a que \[\left|r_n(x)\right|\leq \left|f^{(n+1)}(u)\right|\cdot\frac{\left|x-x_0\right|^{n+1}}{n!}\] pour $u$ entre $x$ et $x_0$.
\end{remark}
...
\begin{itemize}
    \item Calculons la limite suivante : $\lim_{x\to 0}\frac{\sin(x)-x}{x^3}$
        \begin{align*}
            ...
            &=\lim_{x\to 0}\left(-\frac{1}{6}+\varepsilon(x)\right) = -\frac{1}{6}
        \end{align*}
    ...
    \end{align*}
    Cette série converge et la série originale aussi, par comparaison.
\end{itemize}
\begin{example}[Calculs de $DL$]
    Tant qu'on écrit le reste, on peut tout faire.
\end{example}
...
\textbf{Question :} si $f\in C^\infty(I)$, a-t-on $f(x)=\sum_{k=0}^{\infty}\frac{f^{(n)}(x_0)}{k!} (x-x_0)^k$ ? Quand on a $n\to \infty$, il faut que :
\begin{enumerate}
    \item La série converge
    \item Le reste $r_n(x) \longrightarrow 0$ lorsque $n\to \infty$
\end{enumerate}
\begin{definition}[Série de Taylor]
    Pour $f\in C^\infty(I)$ avec $I$ un intervalle ouvert et $x_0\in I$, la série de Taylor de $f$ centrée en $x_0$ est la série \[\sum_{k = 0}^{\infty}\frac{f^{(n)}(x_0)}{k!}(x-x_0)^k\]
\end{definition}
\begin{remark}
...
\end{remark}
\begin{example}[1]
    $f(x) = e^x\in C^\infty(\R), x_0 = 0$. Le $DL_n$ de $f$ donne \[e^x =\sum_{k = 0}^{n}\frac{x^k}{k!}+x^n\varepsilon(x)\] 
    ...
    Car $\exp$ est strictement croissant et que $u\in\left[0, x\right]$, donc $e^u \le e^{\left|x\right|}$. Le reste tend bien vers $0$. Donc $e^x$ est égale à sa série de Taylor en $x_0 = 0$ pour tout $x\in\R$ :
    \[e^x = \sum_{k = 0}^{\infty}\frac{x^k}{k!}\] 
    \begin{remark}
        En fait, $\displaystyle e^x = \sum_{k = 0}^{\infty}\frac{e^{x_0}}{k!}(x-x_0)^k$
    \end{remark}
\end{example}
...
\begin{example}[2]
    ...
    \end{remark}
\end{example}
\begin{proposition}[Dériviées de série entières]
    Si $\displaystyle f(x) = \sum_{k = 0}^{\infty}b_k(x-x_0)^k$, alors \[f'(x) = \sum_{k = 1}^{\infty}k\cdot b_k(x-x_0)^{k-1} = \sum_{k = 0}^{\infty}(k+1)b_{k+1}(x-x_0)^k\] et les rayons de convergences sont les mêmes !
\end{proposition}
\noindent\textbf{Concéquences de la porposition :}
\begin{itemize}
...
\begin{example}[4]
    ...
    \end{align*}
    pour $x\in \left]-1, 1\right[$. En $x=1$, on obtient, par prolongement par continuité que : \[\log(2) = \sum_{k = 1}^{\infty}\frac{(-1)^{k-1}}{k}\]
\end{example}
\begin{example}[5 (Contre example)]
    ...
    \[\lim_{x\to 0}f(x) = e^{-\infty} = 0 = f(0)\] Donc $f$ est continue en $0$. De plus, si $x\neq 0$, $\displaystyle f'(x) = e^{\frac{-1}{x^2}}\frac{2}{x^3}$ et on calcule $\displaystyle\lim_{x\to 0}f'(x)=\lim_{x\to 0}e^{\frac{-1}{x^2}} \frac{2}{x^3}$ :
    \begin{align*}
