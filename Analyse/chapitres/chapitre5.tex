...
\begin{proof}
    ...
    \end{center}
    On voi que dans le cas $(1)$, la fonction converge vers le seul candidat : $l$. Dans le cas $(2)$, la fonction converge aussi. Dans le cas $(3)$, la fonction diverge.
...
\begin{property}[Dériviées de série entières]
    Si $f(x) = \sum_{k = 0}^{\infty}b_k(x-x_0)^k$, alors \[f'(x) = \sum_{k = 0}^{\infty}k\cdot b_k(x-x_0)^{k-1} = \sum_{k = 0}^{\infty}(k+1)b_{k+1}(x-x_0)\] et les rayons de convergences sont les mêmes ! Par example, on a 
    \[e^x = \sum_{k = 0}^{\infty}\frac{x^k }{k!}\]
\end{property}
